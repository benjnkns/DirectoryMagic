...
def copyEmbeddedDirectory(dir_name, dest):
	...
	if size < 3000:
		...
		if os.path.isfile(dir_name):
			...
			for item in os.listdir(dir_name):
				...
				print itempath
				if itemSize + dirSize < 3000:
					dirSize += itemSize
					call(["mkdir", "-p", dest + "/" + dir_name + "-" + str(extension)])
					call(["cp", "-r", itempath, dest + "/" + dir_name + "-" + str(extension)])
				elif itemSize < 3000:
					dirSize = itemSize
					extension += 1
					call(["mkdir", "-p", dest + "/" + dir_name + "-" + str(extension)])
					...
					copyEmbeddedDirectory(itempath, dest)
				...
				if itemSize < 3000:
					...
					for subItem in os.listdir(itempath):
						...
						if subItemSize + totalSubSize < 3000:
							...
							copyEmbeddedDirectory(subItemPath, dest)
